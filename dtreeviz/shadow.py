...
class ShadowDecTree:
    ...
    def get_node_type(self):
        ...
        node_type = np.zeros(shape=self.tree_model.tree_.node_count, dtype=bool)
        stack = [(0)]  # the root node id
        while len(stack) > 0:
            node_id = stack.pop()
            # If we have a split node
            if self.tree_model.tree_.children_left[node_id] != self.tree_model.tree_.children_right[node_id]:
                stack.append((self.tree_model.tree_.children_left[node_id]))
                stack.append((self.tree_model.tree_.children_right[node_id]))
            else:
    ...
    def get_leaf_sample_counts(self):
        ...
        n_node_samples = self.tree_model.tree_.n_node_samples

        leaf_samples = [(i, n_node_samples[i]) for i in range(0, self.tree_model.tree_.node_count) if node_type[i]]
    ...
    def get_leaf_sample_counts_by_class(self):
        ...
        node_type = self.get_node_type()
        sample_values = self.tree_model.tree_.value

        leaf_samples = [(i, sample_values[i][0][0], sample_values[i][0][1])
                        for i in range(0, self.tree_model.tree_.node_count) if (node_type[i])]
